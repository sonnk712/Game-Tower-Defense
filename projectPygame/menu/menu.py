...
star= pygame.transform.scale(pygame.image.load(os.path.join("game_assets","coin.png")),(30, 30))
...
class Menu:
    ...
    def draw(self,win):
        ...
        for item in self.buttons:
            ...
            if item.name=="Upgrade":
                win.blit(star, (item.x + item.width + 5, item.y+5))
                text = self.font.render(str(self.item_cost[self.tower.level - 1]), 1, (255,255,255))
                win.blit(text, (item.x + item.width + 20 - text.get_width()/2, item.y + star.get_height() +10))
            elif item.name=="Sell":
                win.blit(star, (item.x + item.width + 5, item.y+5))
    ...
    def add_sell_btn(self,img,name):
        self.items += 1
        self.buttons.append(SellButton(self, img,name))
...
class VerticalMenu(Menu):
    ...
    def add_btn(self, img, name, cost):
        ...
        """
        self.items += 1
        btn_x = self.x + 200 -(self.items-1)*120
        btn_y = self.y+360 #+ (self.items-1)*120
    ...
    def draw(self, win):
        ...
        """
        #win.blit(self.bg, (self.x - self.bg.get_width()/2, self.y-120))
        win.blit(self.bg, (self.x-25 , self.y+330))
        for item in self.buttons:
            item.draw(win)
            win.blit(star, (item.x+2, item.y + item.height))
